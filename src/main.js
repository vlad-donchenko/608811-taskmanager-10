...
renderTemplate(mainControl, createMainFilterTemplate(), `afterend`);

const mainFilter = main.querySelector(`.main__filter`);
renderTemplate(mainFilter, createBoardTemplate(), `afterend`);

const cardWrapper = main.querySelector(`.board`);
renderTemplate(cardWrapper, createBoardFilterTemplate());

renderTemplate(cardWrapper, createTaskListTemplate());

const taskList = main.querySelector(`.board__tasks`);
renderTemplate(taskList, createCardEditTemplate());

new Array(TASK_COUNT).fill(``).forEach(() => {
  renderTemplate(taskList, createCardTemplate());
});

renderTemplate(taskList, createLoadMoreTemplate(), `afterend`);
